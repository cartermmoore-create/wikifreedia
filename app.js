...
Use == Heading == for sections, * item for lists, and [[Article Name]] for internal article links.`,
    categories:["Wikifreedia","Software"],
    images:[],
    history:[]
  },
  {
...
function loadDb(){
  try{
    ...
    if(x && Array.isArray(x.articles)){
      return x;
    }
  }catch(e){}

  const x = {
    articles: seed.map(a => ({...a, history:[]})),
    drafts:[]
  };
...
function saveDb(){
  try{
    ...
    console.error("Wikifreedia storage error:", e);

    const message =
      "Wikifreedia could not save this change. The browser storage limit may have been reached. Try using smaller images.";
...
function internalLinks(s){
  return esc(s)
    .replace(
      /\[\[([^\]]+)\]\]/g,
      (_,x) =>
        `<a href="#/article/${encodeURIComponent(x)}">${esc(x)}</a>`
    )
    .replace(
      /'''([^']+)'''/g,
      "<strong>$1</strong>"
...
function imageHtml(image){
  ...
  return `
    <figure class="article-image ${align}">
      <img
        src="${image.data}"
        alt="${esc(image.alt || image.caption || "")}"
...
function randomArticle(){
  if(db.articles.length){
    const a =
      db.articles[
        Math.floor(
          Math.random() * db.articles.length
        )
      ];
...
function renderHome(){
  ...
  document.getElementById("view").innerHTML = `
    <section class="hero">
      ...
      <p>
        A Wikipedia-inspired encyclopedia where
        you control what gets written.
        Create an article yourself or use
        the AI writer to get a first draft.
      </p>

      <div class="actions">
        <a class="primary" href="#/create">
          Create an article
        </a>

        <button class="secondary" id="randomBtn">
          Random article
        </button>
      </div>
    </section>
    ...
    <div class="kpis">
      <div class="kpi">
        <strong>${db.articles.length}</strong>
        saved articles
      </div>

      <div class="kpi">
        <strong>${db.drafts.length}</strong>
        drafts
      </div>
      ...
      <div>
        ...
        <div class="box">
          ...
          <p>
            ...
            is the included starter article.
            Edit it, replace it, or use it
            as a template.
          </p>

          <a href="#/article/Wikifreedia">
            Read the Wikifreedia article →
          </a>
            ...
            ${
              newest.map(a => `
                <div class="article-item">
                  ...
                  <h3>
                    <a href="#/article/${encodeURIComponent(a.title)}">
                      ${esc(a.title)}
                    </a>
                  </h3>
...
function renderArticle(title){
  ...
  if(!a){
    ...
    view.innerHTML = `
      ...
      <div class="notice">
        You can create this article now.
      </div>

      <button class="primary" id="makeBtn">
        Create article
      </button>
    `;
...
${esc(a.content)}
      ...
      <div class="media-box">
        ...
        <p class="muted">
          Add or replace images for this article.
        </p>
        ...
        <div
          id="inlineImagePreviewList"
          class="image-preview-list"
        ></div>

        <div
          class="status"
          id="inlineImageStatus"
        ></div>
...
function bindInline(a){
  ...
  renderInlineImagePreview(images);
...
function renderInlineImagePreview(images){
...
function renderEditor(prefill=""){
  ...
  document.getElementById(
    ...
    <section class="article-head">
      ...
      <p class="subtitle">
        Write it yourself or ask
        the AI assistant for a
        starting draft.
      </p>
...
More information."
        ...
        <div class="media-box">
          ...
          <p class="muted">
            Upload images directly in
            your browser. They are saved
            with this article on this device.
          </p>
          ...
          <div class="image-settings">
            ...
            <input
              id="imageCaption"
              placeholder="Caption for the new image"
            >
      ...
      <aside class="ai">
        ...
        <p class="muted">
          ...
          The AI server can generate a draft,
        ...
        <div class="field">
          ...
          <textarea
            ...
            placeholder="Example: Write a neutral encyclopedia article about a fictional city called New Hollow, including its history, geography, culture and notable landmarks."
          ></textarea>
...
function localDraft(prompt){
  ...
  return {
    ...
    summary:
      `An encyclopedia-style draft about ${title}.`,
  ...
  document.getElementById(
    ...
    async () => {
      ...
      try{
        ...
        status(
          "aiStatus",
          "Draft generated. Edit it before publishing."
        );
        ...
        if(
          ...
          status(
            "aiStatus",
            "AI server unavailable; a local demo draft was inserted."
          );
          ...
          status(
            "aiStatus",
            "AI generation failed. Check Settings and the server."
          );
        }
      }
    };
...
function renderHelp(){
  ...
  document.getElementById(
    ...
    >
      ...
      <p>
        This starter version stores
        articles in browser local storage.
        A larger Wikifreedia could use
        accounts, a database, uploads,
        moderation and full revision diffs.
      </p>
...
function settingsModal(){
  ...
  document.getElementById(
    ...
    <div class="notice">
      ...
      never inside browser JavaScript.
